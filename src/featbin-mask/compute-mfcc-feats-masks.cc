...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Create MFCC feature files with frequency and time masks.\n"
        "Usage:  compute-mfcc-feats-masks [options...] <wav-rspecifier> <feats-wspecifier>\n";
    ...
    // Define the frequence and time parameters
    int32 F = 30;
    int32 T = 40;
    int32 mF = 2;
    int32 mT = 2;
    ...
    po.Register("frequency-mask", &F, "The frequency mask parameter F");
    po.Register("time-mask", &T, "The time mask parameter T");
    po.Register("nbr-frequency-mask", &mF, "The number of frequency mask applied");
    po.Register("nbr-time-mask", &mT, "The number time of mask applied");
    ...
    // Data augmentation parameters check
    KALDI_ASSERT(F >= 0);
    KALDI_ASSERT(T >= 0);
    KALDI_ASSERT(mF > 0);
    KALDI_ASSERT(mT > 0);
    ...
    for (; !reader.Done(); reader.Next()) {
      ...
      Matrix<BaseFloat> spectrograms;
      Matrix<BaseFloat> features;
      try { 
        mfcc.ComputeFeaturesMask(waveform, wave_data.SampFreq(), vtln_warp_local, F,T,mF,mT, &features);
      } catch (...) {
