...
// SpecAugmentation implementation
template <class F>
void OfflineFeatureTpl<F>::ComputeFeaturesMask(
    const VectorBase<BaseFloat> &wave,
    BaseFloat sample_freq,
    BaseFloat vtln_warp,
    int32 Fq, int32 Tp, int32 mF, int32 mT,
    Matrix<BaseFloat> *output) {
  KALDI_ASSERT(output != NULL);
  BaseFloat new_sample_freq = computer_.GetFrameOptions().samp_freq;
  if (sample_freq == new_sample_freq) {
    ComputeMask(wave, vtln_warp, Fq, Tp, mF, mT, output);
  } else {
    if (new_sample_freq < sample_freq &&
        ! computer_.GetFrameOptions().allow_downsample)
        KALDI_ERR << "Waveform and config sample Frequency mismatch: "
    ...
    ResampleWaveform(sample_freq, wave,
                     new_sample_freq, &resampled_wave);
    ComputeMask(resampled_wave, vtln_warp, Fq, Tp, mF, mT, output);
  }
}

template <class F>
void OfflineFeatureTpl<F>::ComputeMask(
    const VectorBase<BaseFloat> &wave,
    BaseFloat vtln_warp,
    int32 Fq, int32 Tp, int32 mF, int32 mT,
    Matrix<BaseFloat> *output) {
  ...
  output->Resize(rows_out, cols_out);
  Matrix<BaseFloat> windows;  // all windowed waveform.
  Vector<BaseFloat> log_energy;  // all log energy of the waveform.
  windows.Resize(rows_out,computer_.GetFrameOptions().PaddedWindowSize());
  windows.SetZero();
  log_energy.Resize(rows_out);
  log_energy.SetZero();


  Vector<BaseFloat> window;  // windowed waveform (single frame).
  bool use_raw_log_energy = computer_.NeedRawLogEnergy();
  for (int32 r = 0; r < rows_out; r++) {  // r is frame index.
    ExtractWindow(0, wave, r, computer_.GetFrameOptions(),
                  feature_window_function_, &window,
                  (use_raw_log_energy ? &log_energy(r) : NULL));
    windows.CopyRowFromVec(window,r);
  }
  computer_.ComputeMask(&log_energy, vtln_warp, Fq, Tp, mF, mT, windows, output);
  //KALDI_LOG << output->NumRows();
}
